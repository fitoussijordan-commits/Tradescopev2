...
function parseTradovateCSV(csvText) {
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    // Parse timestamps — FORMAT US: MM/DD/YYYY HH:MM:SS
    ...
    fills.push({
      ...
      durationSec,
      // Date au format YYYY-MM-DD pour matcher avec Supabase
      date: buyTs ? `${buyTs.getFullYear()}-${String(buyTs.getMonth()+1).padStart(2,'0')}-${String(buyTs.getDate()).padStart(2,'0')}` : null,
    });
  }
...
// FORMAT US: "02/16/2026 09:46:04" → MM/DD/YYYY
function parseTimestamp(str) {
  if (!str) return null;
  const match = str.match(/(\d{2})\/(\d{2})\/(\d{4})\s+(\d{2}):(\d{2}):(\d{2})/);
  if (!match) return null;
  const month = parseInt(match[1]) - 1; // MM (0-indexed)
  const day = parseInt(match[2]);        // DD
  const year = parseInt(match[3]);       // YYYY
  return new Date(year, month, day, parseInt(match[4]), parseInt(match[5]), parseInt(match[6]));
}
